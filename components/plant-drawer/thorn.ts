...
export class Thorn {
  ...
  constructor(stage: SVGGElement, position: Position, size: number) {
    this.thorn = document.createElementNS(
    ...
    gsap.to(this.thorn, { duration: 3, scale: 1 });

    stage.appendChild(this.thorn);
  }

  private createThornPath(p: Position, w: number): string {
    return `M ${p.x} ${p.y} Q ${p.x - w / 2} ${p.y}  ${p.x - w / 2} ${p.y + w / 4} L ${p.x} ${p.y + w * 2} L ${p.x + w / 2} ${p.y + w / 4} Q ${p.x + w / 2} ${p.y} ${p.x} ${p.y} Z`;
  }
